...
import com.zhyea.argo.model.request.fci.FciQueryRequest;
...
public class FciService {
	 ...
	 * @return 新增的组件实例id
	 */
	public Long add(FciEntity entity) {
		String fciCode = ShortCode.genUpper();
		FciDto exists = fciMapper.getByCode(fciCode);
		while (null != exists) {
			fciCode = ShortCode.genUpper();
			exists = fciMapper.getByCode(fciCode);
		}
